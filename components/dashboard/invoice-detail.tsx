...
const INVOICE_SECTION_BUTTONS = [
  { key: "overview", label: "الملخص والإيصال" },
  { key: "returns", label: "المرتجع" },
  { key: "admin", label: "الإجراء الإداري" }
] as const satisfies ReadonlyArray<{ key: InvoiceSection; label: string }>;
...
export function InvoiceDetail({ role, invoice, accounts }: InvoiceDetailProps) {
  ...
  const sectionRefs = useRef<Record<InvoiceSection, HTMLButtonElement | null>>({
    overview: null,
    returns: null,
    admin: null
  });
  ...
  const [expiresInHours, setExpiresInHours] = useState(role === "pos_staff" ? "48" : "168");
  ...
  const [isReturnHistoryExpanded, setIsReturnHistoryExpanded] = useState(true);
  ...
  const selectedReturnItems = useMemo(
    () =>
      returnableItems.filter(
        (item) => item.remainingQuantity > 0 && selectedReturnItemIds.includes(item.id)
      ),
    [returnableItems, selectedReturnItemIds]
  );
  ...
  useEffect(() => {
    ...
    setRefundAccountId(accounts[0]?.id ?? "");
    setReturnKey(createUuid());
    setIsReturnHistoryExpanded(invoice.returns.length > 0);
    setActiveSection("overview");
  }, [accounts, invoice.id, invoice.items, invoice.returns.length]);
      ...
      .map((item) => ({
        ...
        quantity:
          returnType === "full"
            ...
            : Math.min(item.remainingQuantity, returnQuantities[item.id] ?? 0)
      }))
  ...
  function handleSectionKeyDown(event: KeyboardEvent<HTMLButtonElement>, currentSection: InvoiceSection) {
    const currentIndex = INVOICE_SECTION_BUTTONS.findIndex((section) => section.key === currentSection);
    if (currentIndex === -1) {
      return;
    }

    const focusSection = (index: number) => {
      const nextSection = INVOICE_SECTION_BUTTONS[index]?.key;
      if (nextSection) {
    ...
    switch (event.key) {
      ...
      case "ArrowDown":
        ...
        focusSection((currentIndex + 1) % INVOICE_SECTION_BUTTONS.length);
        break;
      case "ArrowLeft":
      case "ArrowUp":
        event.preventDefault();
        focusSection((currentIndex - 1 + INVOICE_SECTION_BUTTONS.length) % INVOICE_SECTION_BUTTONS.length);
        break;
      case "Home":
        event.preventDefault();
        focusSection(0);
      ...
      case "End":
        event.preventDefault();
        focusSection(INVOICE_SECTION_BUTTONS.length - 1);
        break;
      case "Enter":
      case " ":
        event.preventDefault();
  ...
  function toggleReturnItem(itemId: string, remainingQuantity: number) {
    const isSelected = selectedReturnItemIds.includes(itemId);

    setSelectedReturnItemIds((current) =>
      isSelected ? current.filter((value) => value !== itemId) : [...current, itemId]
    );
    setReturnQuantities((current) => ({
      ...current,
      [itemId]: isSelected ? 0 : Math.min(Math.max(current[itemId] ?? 1, 1), remainingQuantity)
    }));
  }

  function handleReturnQuantityChange(itemId: string, remainingQuantity: number, nextValue: number) {
    const normalizedValue = Math.min(Math.max(nextValue, 0), remainingQuantity);
  ...
  function handleCreateReceiptLink() {
    ...
    startTransition(() => {
      void (async () => {
        ...
        setRetryAction(null);
        toast.success(
          envelope.data.is_reissued ? "تم تحديث رابط الإيصال الحالي." : "تم إنشاء رابط الإيصال العام."
        );
      })();
    });
  }
  ...
  function renderInvoiceSummaryCard(contextTitle: string) {
    return (
      <SectionCard eyebrow="تفاصيل الفاتورة" title={invoice.invoice_number} className="transaction-card">
        <div className="invoice-page__context-header">
          <h2>{contextTitle}</h2>
          <span className="badge badge--info">{getInvoiceStatusLabel(invoice.status)}</span>
        </div>
        ...
        <div className="info-strip">
          ...
          {invoice.customer_phone ? <span>الهاتف: {invoice.customer_phone}</span> : null}
        </div>
          ...
          {invoice.items.map((item) => {
            ...
            return (
              <article key={item.id} className="list-card">
                ...
                <p>
                  الكمية: {formatCompactNumber(item.quantity)} | المرتجع: {formatCompactNumber(item.returned_quantity)} | المتبقي:{" "}
          ...
          <div className="stack-list">
            {invoice.payments.map((payment) => (
              <article key={payment.id} className="list-card">
                ...
                <p>
                  صافي: {formatCurrency(payment.net_amount)} | رسوم: {formatCurrency(payment.fee_amount)} | التاريخ:{" "}
  ...
  return (
    <section className="workspace-stack transaction-page invoice-page invoice-detail-page">
      <PageHeader
        ...
        meta={
          <div className="transaction-page__meta" aria-label="ملخص الفاتورة">
            ...
            <article className="transaction-page__meta-card stat-card">
              <span>الحالة</span>
              <strong className="badge badge--info">{getInvoiceStatusLabel(invoice.status)}</strong>
            </article>
            <article className="transaction-page__meta-card transaction-page__meta-card--safe stat-card">
      ...
      <div className="inline-actions">
        ...
        <button type="button" className="secondary-button btn btn--secondary" onClick={() => window.print()}>
          <Printer size={16} />
          طباعة الإيصال
        </button>
      </div>
      ...
      <div className="chip-row transaction-chip-row invoice-page__sections" aria-label="أقسام تفاصيل الفاتورة">
        {INVOICE_SECTION_BUTTONS.map((section) => (
          <button
            key={section.key}
            ref={(node) => {
            ...
            aria-controls={`invoice-panel-${section.key}`}
            className={activeSection === section.key ? "chip-button is-selected" : "chip-button"}
            onClick={() => activateSection(section.key)}
            onKeyDown={(event) => handleSectionKeyDown(event, section.key)}
          >
            {section.label}
          </button>
        ))}
      ...
      <section
        id="invoice-panel-overview"
        className="invoice-page__section"
        role="region"
        aria-labelledby="invoice-section-overview"
        hidden={activeSection !== "overview"}
      >
        <div className="invoice-page__primary">{renderInvoiceSummaryCard("الملخص والإيصال")}</div>
      ...
      <section
        id="invoice-panel-returns"
        className="invoice-page__section"
        role="region"
        aria-labelledby="invoice-section-returns"
        hidden={activeSection !== "returns"}
      >
        <div className="invoice-page__primary">
          {renderInvoiceSummaryCard("المرتجع")}
        ...
        <div className="invoice-page__secondary">
          <div className="invoice-page__secondary-panel">
            ...
            <div className="invoice-page__return-items">
              {returnableItems.some((item) => item.remainingQuantity > 0) ? (
                returnableItems
                  .filter((item) => item.remainingQuantity > 0)
                  .map((item) => {
                    const isSelected = selectedReturnItemIds.includes(item.id);

                    return (
                      <label
                        key={item.id}
                        className={`invoice-page__return-item ${isSelected ? "is-selected" : ""}`}
                      >
                        <div className="invoice-page__return-item-main">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => toggleReturnItem(item.id, item.remainingQuantity)}
                          />
                          <div>
                            <strong>{item.product_name_at_time}</strong>
                            <p className="workspace-footnote">
                              المتاح للإرجاع: {formatCompactNumber(item.remainingQuantity)}
                            </p>
                          </div>
                        </div>
                        <span>{formatCurrency(item.total_price)}</span>
                ...
                <div className="empty-panel invoice-page__return-empty">
                  <RotateCcw size={18} />
                  <p>لا توجد بنود قابلة للإرجاع في هذه الفاتورة.</p>
                </div>
              )}
            </div>
          </div>

          <div className="invoice-page__return-form" hidden={selectedReturnItems.length === 0}>
            <h3 className="invoice-page__secondary-title">إعداد المرتجع</h3>
                ...
                <article key={item.id} className="list-card">
                  ...
                  <label className="stack-field">
                    ...
                    <input
                      ...
                      step={1}
                      value={returnType === "full" ? item.remainingQuantity : returnQuantities[item.id] ?? 1}
                      disabled={returnType === "full"}
                      onChange={(event) =>
                        handleReturnQuantityChange(item.id, item.remainingQuantity, Number(event.target.value))
                      }
                    />
                  </label>
                </article>
            ...
            <div className="stack-form">
              ...
              <div className="info-strip">
                <span>تم تحديد {formatCompactNumber(selectedReturnItems.length)} بند للمرتجع.</span>
                <span>يتم تحديث الفاتورة والمخزون والحساب المرتبط تلقائيًا بعد التنفيذ.</span>
              </div>
          ...
          <div className="invoice-page__secondary-panel">
            <button
              type="button"
              className="invoice-page__secondary-title invoice-page__secondary-toggle"
              aria-expanded={isReturnHistoryExpanded}
              onClick={() => setIsReturnHistoryExpanded((current) => !current)}
            >
              <span>سجل الإرجاعات</span>
              <ChevronDown
                size={18}
                className={isReturnHistoryExpanded ? "invoice-page__history-icon is-rotated" : "invoice-page__history-icon"}
              />
            </button>

            <div className="invoice-page__return-history" hidden={!isReturnHistoryExpanded}>
              {historicalReturns.length > 0 ? (
                ...
                <div className="empty-panel invoice-page__return-empty">
                  <FileText size={18} />
                  <p>لا توجد عمليات إرجاع سابقة لهذه الفاتورة.</p>
                </div>
              )}
            </div>
          </div>
      ...
      <section
        id="invoice-panel-admin"
        className="invoice-page__section"
        role="region"
        aria-labelledby="invoice-section-admin"
        hidden={activeSection !== "admin"}
      >
        <div className="invoice-page__primary">{renderInvoiceSummaryCard("الإجراء الإداري")}</div>

        <div className="invoice-page__secondary">
          {role === "admin" ? (
            <div className="invoice-page__danger-panel" aria-live="polite">
              <h3 className="invoice-page__danger-title">إجراء خطر: إلغاء الفاتورة</h3>
              <p className="workspace-footnote">
                إلغاء الفاتورة سيؤثر على الأرصدة والسجلات المرتبطة بها. راجع السياسة الداخلية قبل التنفيذ.
              </p>
            ...
            <div className="invoice-page__danger-panel">
              <h3 className="invoice-page__danger-title">مقيد</h3>
